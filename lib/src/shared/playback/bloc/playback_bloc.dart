...
class PlaybackBloc extends Bloc<PlaybackEvent, PlaybackState> {
  ...
  Stream<Duration> _startPlayingStream() async* {
    while (state.songWithProgress!.progress < state.songWithProgress!.song.length) {
      await Future<void>.delayed(_playbackUpdateInterval);
      yield _playbackUpdateInterval;
      if (state.songWithProgress!.progress >= state.songWithProgress!.song.length) {
  ...
  void _resumePlayback() =>
      _currentlyPlayingSubscription = _startPlayingStream().listen(_handlePlaybackProgress);
  ...
  void _moveToInSong(MoveToInSong event, Emitter<PlaybackState> emit) {
    _pausePlayback();
    final targetMilliseconds = state.songWithProgress!.song.length.inMilliseconds * event.percent;
    emit(
      state.copyWith(
        isPlaying: false,
        songWithProgress: state.songWithProgress!.copyWith(
