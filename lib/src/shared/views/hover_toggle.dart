...
class _HoverToggleState extends State<HoverToggle> with MaterialStateMixin {
  ...
  Widget build(BuildContext context) {
    return SizedBox.fromSize(
      ...
      child: MouseRegion(
        ...
        child:
            widget.mode == HoverMode.replace ? _buildReplaceableChildren() : _buildChildrenStack(),
      ),
    );
  }

  Widget _buildChildrenStack() {
    Widget child = isHovered ? Opacity(opacity: 0.2, child: widget.child) : widget.child;
    return Stack(
      children: <Widget>[
        child,
  ...
  Widget _buildReplaceableChildren() => isHovered ? widget.hoverChild : widget.child;
}
